...
const StageProvider: ParentComponent<{ initialState?: Partial<StageState> }> = (
  props
) => {
...
function Stage(props: { class?: string; initialState?: Partial<StageState> }) {
  return (
    <StageProvider initialState={props.initialState}>
...
function ElementRenderer({
  ...
  elementId: string;
  renderableElements: Record<string, ValidComponent>;
}) {
  const { state } = useStage();
  const element = () => state.elements[elementId];
  return (
    <Show when={element()}>
      <For each={Object.entries(renderableElements)}>
        {([type, el]) => (
          <Show when={element().type === type}>
            <Dynamic component={el} elementId={elementId} element={element} />
          </Show>
        )}
      </For>
    </Show>
...
function CircleElement({ elementId }: { elementId: string }) {
  const { state } = useStage();
  return (
    <>
      <div
        style={{
          "background-color": state.elements[elementId].props.color,
...
function RectangleElement({ elementId }: { elementId: string }) {
  const { state } = useStage();
  return (
    <>
      <div
...
function ElementTransformControls({ elementId }: { elementId: string }) {
